...
namespace CapaPresentacion
{
    public partial class formAlumnos : Form
    {
        ...
        {
            dataListadoAlumnos.DataSource = objetoCN.MostrarAlumnos();
            dataListadoAlumnos.Columns[0].Visible = false;
            // lblTotalAlumnos.Text = "Total de Registros: " + Convert.ToString(dataListadoClientes.Rows.Count);
        ...
        private void dataListadoAlumnos_SelectionChanged(object sender, EventArgs e)
        {
            if (dataListadoAlumnos.SelectedCells.Count > 0)
            {
                int selectedrowindex = dataListadoAlumnos.SelectedCells[0].RowIndex;
                DataGridViewRow selectedRow = dataListadoAlumnos.Rows[selectedrowindex];
                this.IdAlumno = Convert.ToInt32(selectedRow.Cells["IdAlumno"].Value);
                Console.WriteLine("El id IdAlumno es " + this.IdAlumno);
            }
        }
